...
from .serializers import (
    ...
    SubscriberListSerializer, EmailEngagementSerializer,
)
...
@extend_schema(tags=['Subscriber Lists'])
class SubscriberListListCreateView(generics.ListCreateAPIView):
...
@extend_schema(tags=['Subscriber Lists'])
class SubscriberListDetailView(generics.RetrieveUpdateDestroyAPIView):
    queryset = SubscriberList.objects.all()
    serializer_class = SubscriberListSerializer
    permission_classes = [IsAuthenticated]
    lookup_field = 'pk'
...
@extend_schema(tags=['Email Engagement'])
class EmailEngagementReportView(generics.ListAPIView):
    queryset = EmailCampaign.objects.filter(status='sent').all()
