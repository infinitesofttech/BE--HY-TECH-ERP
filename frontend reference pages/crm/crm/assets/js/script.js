...
Template Name: Preadmin CRM - Bootstrap Admin Template
*/
...
(function () {
	...
	$(document).on('click', '.add-info-fieldset .wizard-next-btn', function () {
		...
		$progressBar.find('.active').removeClass('active').addClass('activated').next().addClass('active');
	});
...
$(document).ready(function () {
    ...
    function loadPeriods(container, type, selectedValue = null) {
        ...
        let periodSelect = container.find(".period-select");
        let periodWrapper = container.find(".period-wrapper");
        let currentYear = new Date().getFullYear();

        periodSelect.empty();

        if (!type) {
            periodWrapper.addClass("d-none");
            return;
        }

        periodWrapper.removeClass("d-none");
    ...
    $(document).on("change", ".period-type", function () {

        let container = $(this).closest(".period-container");
        let type = $(this).val();
    ...
    window.setEditPeriod = function(containerSelector, type, value) {
        ...
        container.find(".period-type").val(type);
